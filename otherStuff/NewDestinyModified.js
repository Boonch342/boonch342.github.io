...
DecideDestiny.AllDestinies = [
	...
	{name:'Dragon Harvest',   icon:[2, 25], effect:'dragon harvest',   prereq:'Destiny: Fantasy'},
	{name:'Dragonflight',     icon:[0, 25], effect:'dragonflight',     prereq:'Destiny: Fantasy'},
	{name:'Cookie chain',     icon:[20, 14], effect:'chain cookie',     prereq:'Destiny: Scattershot'},
	{name:'Cookie storm',     icon:[22, 6], effect:'cookie storm',     prereq:'Destiny: Scattershot'},
	{name:'Click frenzy',     icon:[12, 14], effect:'click frenzy',     prereq:'Destiny: Carpal tunnel'},
	...
	{name:'Hurri-Cane Sugar',	icon:[29, 16], effect:'hurricane sugar',	prereq:'Destiny: Sugar rush'},
	{name:'Sweet',		icon:[29, 27], effect:'free sugar lump',	prereq:'Destiny: Sugar rush'},
];
...
DecideDestiny.CreateUpgrades = function(){
	...
	ups.push(CCSE.NewHeavenlyUpgrade('Destiny: Fantasy',   loc("Adds the <b>Dragon harvest</b> and <b>Dragonflight</b> effects to the pool of choices."),                                                              1e5,  [10,25], 570, -330, ['Destiny: Architecture']));
	ups.push(CCSE.NewHeavenlyUpgrade('Destiny: Scattershot',   loc("Adds the <b>Cookie chain</b> and <b>Cookie storm</b> effects to the pool of choices."),                                       1e6,  [22, 6], 670, -380, ['Destiny: Fantasy']));
	...
	ups.push(CCSE.NewHeavenlyUpgrade('Destiny: Apocalypse',      loc("Adds the <b>Elder frenzy</b> effect to the pool of choices."),                                                                1e10, [29, 6], 970, -530, ['Destiny: Misfortune']));
	ups.push(CCSE.NewHeavenlyUpgrade('Destiny: Whimsy',    loc("Adds the <b>Blab</b> effect to the pool of choices."),                                                                1e12, [29, 8], 1070, -580, ['Destiny: Apocalypse']));
	ups.push(CCSE.NewHeavenlyUpgrade('Destiny: Sugar rush',        loc("Adds the <b>Hurri-Cane sugar</b> and <b>Sweet</b> effects to the pool of choices."),                                                                        1e15, [29, 14], 1270, -580, ['Destiny: Whimsy']));
...
DecideDestiny.CreateAchievements = function(){
	...
	last = CCSE.NewAchievement('Decisive', 'Decide your destiny <b>once</b>.', [22,11]); last.order = order; order += 0.001;
	last = CCSE.NewAchievement('Control freak', 'Decide your destiny <b>10 times</b> in one ascension.', [22,11]); last.order = order; order += 0.001;
		last.pool = 'shadow';
...
DecideDestiny.calcCost = function(){
	return Math.pow(1, DecideDestiny.timesDecided);
}
